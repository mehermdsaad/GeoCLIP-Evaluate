...
class GeoCLIP(nn.Module):
    ...
    def forward(self, image, location, texts=None):
        ...
        image_features = self.image_encoder(image, texts)
    ...
    @torch.no_grad()
    def predict(self, image_path, top_k, texts=None):
        ...
        gps_gallery = self.ready_gps_gallery

        logits_per_image = self.forward(image, gps_gallery, texts)
    ...
    @torch.no_grad()
    def heatmap_from_text(self, image_path, text):
        """Given some text, this function returns the cosine similarity score along with the gps coordinates

        Args:
            image_path(str): redundant arg to not break the code. Unnecessary for the method's calculations
            text(str): text used to make the query

        Returns:
            gps_coordinates: The list of gps coordinates
            cosine_similarities: The similarity score of these coordinates with respect to the text.
        """

        gps_gallery = self.gps_gallery.to(self.device)

        # REDUNDANT, BUT NEED IT FOR NOW TO JUST RUN THE DAMN THING
        image = Image.open(image_path)
        image = self.image_encoder.preprocess_image(image)
        image = image.to(self.device)

        logits_per_image = self.forward(image, gps_gallery, texts=text)

        return gps_gallery, logits_per_image
